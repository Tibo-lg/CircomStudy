...
include "../../RangeProof/contracts/circuits/RangeProof.circom";
...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProof[i][j][0] = RangeProof(64);
            rangeProof[i][j][0].in <== puzzle[i][j];
            rangeProof[i][j][0].range[0] <== -1;
            rangeProof[i][j][0].range[1] <== 10;
            rangeProof[i][j][0].out === 1;
            rangeProof[i][j][1] = RangeProof(64);
            rangeProof[i][j][1].in <== solution[i][j];
            rangeProof[i][j][1].range[0] <== -1;
            rangeProof[i][j][1].range[1] <== 10;
